...
def get_dates(date1, date2, intv):
  sdatestr = '%sT%s0000' %(date1[0:8], date1[8:10])
  edatestr = '%sT%s0000' %(date2[0:8], date1[8:10])
...
dates = get_dates(date1, date2, 6)
#if '2019092712' in dates: dates.remove('2019092712')
deltap = 50.; pbot = 975
nlevs = 23
...
for date in dates:
   #times[ndate] = dateutils.datetohrs(date)
    times[ndate] = get_datetohrs(date)
    if runid != '': 
