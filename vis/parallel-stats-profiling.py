  ...
  def process(self):
    ...
    for n in range(len(self.nodelist)):
     ...
     #          self.nodelist[n], self.corelist[n])
      rundir = '%s/run_80.36t%dn_%dp' %(self.workdir,
                self.nodelist[n], self.corelist[n])
     #flnm = '%s/stdoutNerr/stdout.00000000' %(rundir)
     #flnm = '%s/stdoutNerr.observer/stdout.00000000' %(rundir)
     #flnm = '%s/stdoutNerr.solver/stdout.00000000' %(rundir)
     #flnm = '%s/log.jedi_allobs' %(rundir)
     #flnm = '%s/log.allobs_JEDI_full_run_anna2' %(rundir)
     #flnm = '%s/log.observer' %(rundir)
     #flnm = '%s/log.allobs_RR_anna1' %(rundir)
      flnm = '%s/log.allobs_RR_anna2' %(rundir)
    ...
    for k in range(kl):
     ...
     #print('stats.keys() = ', stats.keys())
      if('util::Timers::Total' in stats.keys()):
        print('stats[util::Timers::Total] = ', stats['util::Timers::Total'])
